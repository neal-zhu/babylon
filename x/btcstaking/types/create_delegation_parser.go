...
func ParseCreateDelegationMessage(msg *MsgCreateBTCDelegation) (*ParsedCreateDelegationMessage, error) {
	...
	if err != nil {
		fmt.Println("Error parsing staking tx proof of inclusion:", err)
	...
	if err != nil {
		fmt.Println("Error deserializing staking tx:", err)
	...
	if err != nil {
		fmt.Println("Error deserializing staking slashing tx:", err)
	...
	if err != nil {
		fmt.Println("Error deserializing unbonding tx:", err)
	...
	if err != nil {
		fmt.Println("Error deserializing unbonding slashing tx:", err)
	...
	if msg.UnbondingTime > math.MaxUint16 {
		fmt.Println("Error unbonding time:", msg.UnbondingTime)
	...
	if msg.StakingTime > math.MaxUint16 {
		fmt.Println("Error staking time:", msg.StakingTime)
	...
	if err != nil {
		fmt.Println("Error parsing staker staking slashing signature:", err)
	...
	if err != nil {
		fmt.Println("Error parsing staker unbonding slashing signature:", err)
	...
	if err != nil {
		fmt.Println("Error parsing finality provider public keys:", err)
		//return nil, fmt.Errorf("failed to parse finality provider public keys: %v", err)
	}

	if ExistsDup(fpPKs.PublicKeysBbnFormat) {
		fmt.Println("Error duplicated finality provider public keys")
		//return nil, ErrDuplicatedFp
	}

	if len(fpPKs.PublicKeysBbnFormat) != 1 {
		fmt.Println("Error too many finality provider public keys")
